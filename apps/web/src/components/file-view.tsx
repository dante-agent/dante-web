...
function CodePane({
  ...
  /** 모델에 지금 들어 있는 텍스트(우리가 넣은 그대로). path 가 바뀌면 에디터가 모델을 갈아 끼우므로 모델과 함께 기억한다. */
  const shownRef = useRef<string | null>(null);
  const shownModelRef = useRef<unknown>(null);
  const onTypedRef = useRef(onTyped);
  useEffect(() => {
    onTypedRef.current = onTyped;
  });

  const options = useMemo(() => ({ ...OPTIONS, ariaLabel: label }), [label]);
  ...
  const write = useCallback(
    (text: string, reveal: boolean) => {
      ...
      // 다른 모델로 바뀌었으면(다른 파일, 전에 열었던 모델 재사용) 그 모델의 실제 내용이 기준이다.
      if (shownModelRef.current !== model) {
        shownModelRef.current = model;
        shownRef.current = model.getValue();
      }
  ...
  }, [typing, value, follow, write, path]);
...
export function FileView({
  ...
  const afterLabel = `After: ${testName}. Press Ctrl+M (Ctrl+Shift+M on Mac) to move focus with Tab.`;
  const editOptions = useMemo(() => ({ ...EDIT_OPTIONS, ariaLabel: afterLabel }), [afterLabel]);
  const diffOptions = useMemo(
    () => ({
      ...DIFF_OPTIONS,
      originalAriaLabel: `Before: ${testName}`,
      modifiedAriaLabel: afterLabel,
    }),
    [testName, afterLabel]
  );
