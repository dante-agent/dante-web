...
/**
 * 찾기 input + 목록 + footer. 데스크톱 팝오버(HeaderSwitcher)와 모바일 바텀시트(PathSheet)가
 * 같은 걸 쓴다 — 목록 모양이 두 곳에서 갈라지지 않게.
 */
export function SwitcherList({
  ...
  return (
    <>
      ...
      <ul className={cn("max-h-64 overflow-y-auto p-1", listClassName)}>
        {filtered.map((i) => (
          <li key={i.value}>
            <button
              type="button"
