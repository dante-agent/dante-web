"use client";

import { useState, type ReactNode } from "react";
import { PanelLeftClose, PanelLeftOpen } from "lucide-react";
import { cn } from "@/lib/utils";

// 폴더 보기 / AI 추천 섹션의 서브 사이드바 셸.
// 메인 레일과 같은 패턴: fixed(콘텐츠 위로 덮음), left-14(접힌 레일 폭만큼 띄움). z-20 < 레일 z-30.

const BTN =
  "text-sidebar-foreground/70 hover:text-sidebar-foreground grid place-items-center transition-colors";

export function SubSidebar({ nav, children }: { nav?: ReactNode; children: ReactNode }) {
  const [collapsed, setCollapsed] = useState(false);
  const toggle = () => setCollapsed((c) => !c);

  return (
    <>
      <aside
        className={cn(
          "fixed top-12 bottom-0 left-14 z-20 overflow-hidden transition-[width] duration-200",
          collapsed ? "w-11" : "border-sidebar-border bg-sidebar w-60 border-r"
        )}
      >
        {collapsed ? (
          <div className="flex h-full flex-col items-start pt-2 pl-2">
            <button
              type="button"
              onClick={toggle}
              aria-label="서브 사이드바 펼치기"
              className={cn(BTN, "size-8")}
            >
              <PanelLeftOpen className="size-4" />
            </button>
          </div>
        ) : (
          <div className="relative flex h-full w-60 flex-col p-3">
            <button
              type="button"
              onClick={toggle}
              aria-label="서브 사이드바 접기"
              className={cn(BTN, "absolute top-2 right-1 h-9 w-10")}
            >
              <PanelLeftClose className="size-4" />
            </button>
            {nav}
          </div>
        )}
      </aside>

      <div className={cn("transition-[margin] duration-200", collapsed ? "ml-7" : "ml-60")}>
        {children}
      </div>
    </>
  );
}
