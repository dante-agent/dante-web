...
import { Check, ChevronDown, ChevronRight, Copy, FlaskConical } from "lucide-react";
import { DiffView } from "./diff-view";
import { TestTerminal, type TestRunView } from "./test-terminal";
    ...
    <section className="bg-background flex min-w-0 flex-1 flex-col">
      ...
      <div className="border-border flex h-12 shrink-0 items-center gap-1 border-b px-3">
        <button
          ...
          className={
            tab === "code"
              ...
              : "text-muted-foreground hover:text-foreground px-3 py-1.5 text-sm"
          }
        >
          Code
        </button>
        <button
          ...
          className={
            tab === "terminal"
              ? "bg-muted flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium"
              : "text-muted-foreground hover:text-foreground flex items-center gap-1.5 px-3 py-1.5 text-sm"
          }
        >
      ...
      {tab === "terminal" ? (
        <TestTerminal projectRef={projectRef} versionId={versionId} initialRun={initialRun} />
      ) : (
        <>
          {/* Review 서브헤더 */}
          ...
          <div className="border-border flex h-9 shrink-0 items-center gap-2 border-b px-4">
            ...
            <span className="text-brand-cobalt font-mono text-xs font-semibold">
              {code.changeType}
            </span>
