  ...
  return (
    <Card className="flex h-full flex-col">
      ...
      <CardContent className="flex flex-1 flex-col justify-end">
        <div className="grid grid-flow-col grid-rows-[repeat(7,11px)] gap-[3px] overflow-x-auto">
          {levels.map((level, i) => (
            <div
              key={i}
              className={`bg-brand-mint size-[11px] shrink-0 rounded-[2px] ${LEVEL_OPACITY[level] ?? LEVEL_OPACITY[0]}`}
            />
          ))}
        </div>
        <div className="text-muted-foreground mt-3 flex items-center justify-end gap-1.5 text-[11px]">
          ...
          {LEVEL_OPACITY.map((op) => (
            <span key={op} className={`bg-brand-mint size-[11px] rounded-[2px] ${op}`} />
          ))}
          <span>많음</span>
        </div>
      </CardContent>
