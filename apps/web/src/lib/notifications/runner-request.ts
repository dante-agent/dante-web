// runner(apps/runner) 에 보낼 값을 다듬는 규칙. 순수 함수다.

export type RunnerFramework = "vitest" | "jest";

/** 프로젝트의 testFramework 중 runner 가 받는 값만. 고르지 않았거나 모르는 값이면 null */
export function runnerFramework(value: string | null): RunnerFramework | null {
  return value === "vitest" || value === "jest" ? value : null;
}

/**
 * runner 는 테스트 커맨드 뒤에 파일 경로와 리포트 플래그를 붙인다
 * (apps/runner/src/report.ts 의 buildTestCommand).
 *
