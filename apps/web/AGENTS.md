...
- 비밀값은 평문 저장 금지. Node 내장 `crypto` 로 AES-256-GCM (`ENCRYPTION_KEY`, `src/lib/crypto/secret.ts`). 지금 암호화해 저장하는 값은 채팅 대화 제목·본문, Slack 봇 토큰, Discord 웹훅 URL 이다. 사용자 API 키는 없고(Dante 가 프로바이더와 직접 계약) GitHub 은 설치 토큰을 매번 새로 발급받는다. AI 제공자 키는 서버 환경변수로만 둔다.
